...
lista = np.array([[]])
lista.resize((3500,23))
rezultate = np.array([])
rezultate.resize((3500))
Date=open("hypothyroid2.data","r")
dparcurs=0
...
tfmodel.add(tf.keras.layers.Dense(512,activation='hard_sigmoid'))
tfmodel.add(tf.keras.layers.Dense(512,activation='hard_sigmoid'))
...
tfmodel.fit(lista,rezultate,epochs=2,validation_split=0.04)
