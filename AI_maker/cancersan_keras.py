...
with tf.device('/gpu:0'):
    ...
    date1 = keras_data.flow_from_directory(path1, target_size = (50, 50),batch_size=37, classes = ["normal","cancerigen"], class_mode = "binary")
    path2="D:\\tiroida\\cancer_san\\test"
    date2 = keras_data.flow_from_directory(path2, target_size = (50, 50),batch_size=20, classes = ["normal","cancerigen"], class_mode = "binary")
    ...
    tfmodel.add(Conv2D(filters=4, kernel_size=(3,3), activation="relu",padding='same'))
    tfmodel.add(Conv2D(filters=4, kernel_size=(3,3), activation="relu",padding='same'))
